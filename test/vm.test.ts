...
import { perc_number, perc_string, perc_nil, perc_closure } from "../src/vm/perc_types.ts";
...
describe('PerC Virtual Machine', () => {
    const runVM = (code: opcode[], setup?: (vm: VM) => void) => {
        ...
        return { vm, lastStackVal, printed, steps, done: result.done, error };
    };
    ...
    it('should calculate Fibonacci(6) recursively', () => {
        const fib_code: opcode[] = [
            ...
            /* 2 */ { type: 'load', name: 'fib', src_start: 0, src_end: 0 },
            /* 3 */ { type: 'push', imm: new perc_number(6), src_start: 0, src_end: 0 },
            ...
            /* 13*/ { type: 'load', name: 'fib', src_start: 0, src_end: 0 },
            /* 14*/ { type: 'load', name: 'n', src_start: 0, src_end: 0 },
            /* 15*/ { type: 'push', imm: new perc_number(1), src_start: 0, src_end: 0 },
            /* 16*/ { type: 'binary_op', op: '-', src_start: 0, src_end: 0 },
            /* 17*/ { type: 'call', nargs: 1, src_start: 0, src_end: 0 },

            /* 18*/ { type: 'load', name: 'fib', src_start: 0, src_end: 0 },
            /* 19*/ { type: 'load', name: 'n', src_start: 0, src_end: 0 },
            /* 20*/ { type: 'push', imm: new perc_number(2), src_start: 0, src_end: 0 },
            /* 21*/ { type: 'binary_op', op: '-', src_start: 0, src_end: 0 },
    ...
    describe('Reference Semantics', () => {
        ...
        it('should error when using ref on non-reference types', () => {
            ...
            const { error } = runVM(code);
            expect(error).not.toBeNull();
            expect(error.value).toContain("Cannot ref a non-reference type");
            expect(error.location).toEqual([10, 20]);
        });

        it('should error when using ref on tuple (primitive type)', () => {
            // init x = (| 1, 2 |)
            // ref y = x -> Error
            const code: opcode[] = [
                { type: 'push', imm: new perc_number(1), src_start: 0, src_end: 0 },
                { type: 'push', imm: new perc_number(2), src_start: 0, src_end: 0 },
                { type: 'new_tuple', size: 2, src_start: 0, src_end: 0 },
                { type: 'init', name: 'x', catch: false, src_start: 0, src_end: 0 },
                { type: 'load', name: 'x', src_start: 0, src_end: 0 },
                { type: 'ref', name: 'y', catch: false, src_start: 10, src_end: 20 }
            ];

            const { error } = runVM(code);
            expect(error).not.toBeNull();
            expect(error.value).toContain("Cannot ref a non-reference type: tuple");
        });
    });
});
